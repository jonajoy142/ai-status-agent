...
export default function WorkItemsPage() {
  ...
  const rows = items || fallbackItems.map((item) => ({ id: item.id, external_id: item.id, title: item.title, status: item.status, priority: item.priority, assignee: item.assignee, sprint: item.sprint, source: item.source, risk_level: item.risk.toLowerCase() as "low" | "medium" | "high" | "critical", stale_score: item.stale, business_impact: item.impact, suggested_next_action: item.next }));
